...
class SaleExcelReportController(http.Controller):
    ...
    def get_sale_excel_report(self, wizard=None, **args):
        ...
        for user in wizard.user_id:
            ...
            sheet.merge_range('A1:I1', 'Sales Report in Excel Format', title_style)
            ...
            sheet.write(1, 5, 'Product', header_style)
            sheet.write(1, 6, 'Internal Reference', header_style)
            sheet.write(1, 7, 'Quantity', header_style)
            sheet.write(1, 8, 'Price', header_style)
            ...
            orders = request.env['sale.order'].search(
                [('user_id', '=', user.id), ('date_order', '>=', wizard.start_date),
            ...
            for order in orders:
                ...
                for line in order.order_line:
                    sheet.write(row, 5, line.product_id.name, text_style)
                    sheet.write(row, 6, line.product_id.default_code, text_style)
                    sheet.write(row, 7, line.product_uom_qty, number_style)
                    sheet.write(row, 8, line.price_unit, number_style)
                    row += 1
                    number += 1
            ...
            sheet.merge_range('A' + str(row + 1) + ':H' + str(row + 1), 'Total', text_style)
            sheet.write_formula(row, 8, '=SUM(E3:E' + str(row) + ')', number_style)
